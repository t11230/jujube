...
namespace MusicSelect {
    namespace {
        PanelLayout get_panel_layout(const Data::SongList& song_list, ScreenResources& t_resources, SongOrdering ordering) {
            switch (ordering) {
            case SongOrdering::Title:
                return PanelLayout::title_sort(song_list, t_resources);
            case SongOrdering::Level:
                return PanelLayout::level_sort(song_list, t_resources);
            }
            throw std::runtime_error("Unknown PanelOrdering");
        }
    }

    Screen::Screen(const Data::SongList& t_song_list, ScreenResources& t_resources) :
        HoldsResources(t_resources),
        song_list(t_song_list),
        ribbon(get_panel_layout(t_song_list, t_resources, song_ordering), t_resources),
    ...
    std::optional<Data::SongDifficulty> Screen::select_chart(sf::RenderWindow& window) {
        ...
        sort_button.setPosition(get_ribbon_x(), get_ribbon_y());
    ...
    void Screen::draw_debug(sf::RenderWindow& window) {
        if (debug) {
    ...
    void Screen::handle_key_press(const sf::Event::KeyEvent& key_event, sf::RenderWindow& window) {
    ...
    void Screen::handle_mouse_click(const sf::Event::MouseButtonEvent& mouse_button_event) {
    ...
    void Screen::press_button(Input::Button button) {
        shared.button_highlight.button_pressed(button);

        switch (button) {
        case Input::Button::B1: // Sort Menu
            if (song_ordering == SongOrdering::Title) {
                song_ordering = SongOrdering::Level;
            } else {
    ...
    void Screen::cycle_display_styles(sf::RenderWindow& window) {
    ...
    void Screen::Screen::update_view(sf::RenderWindow& window, sf::View view) {
        window.setView(view);
